...
# 참값 만들고 오염 주입. 정제는 안 함 -> 정제 전 단계에서 이상치 규칙이 오작동과 진짜 고장을 구분할 수 있는가
...
# 이 값이 속한 연속 구간이 몇 분짜리인가 -> 3분 이상일 경우에만 True
...
# A4-1: AI4I에 IQR 적용 → 이상치=고장 신호였다는 걸 발견
# A4-2: 실제로 지워서 학습 → train 고장 건수 감소로 재현율 하락 실측
# A4-3: ★ 시뮬레이터로 검증 → AI4I와 정반대! 이상치 대부분이 센서오작동
# A4-4: 지속시간·전류정합 → "여러 센서 정합성"이 신뢰할 만한 대안
# A4-5: 그림 → 시각적 증거
